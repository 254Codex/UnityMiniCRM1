...
python manage.py dbshell --command="SELECT version_num FROM alembic_version;"

# 6. Monitor performance after migration
python manage.py dbshell --command="EXPLAIN ANALYZE SELECT * FROM companies;"
```
...
CREATE INDEX idx_deal_stage ON deals(stage, status);
CREATE INDEX idx_task_due ON tasks(due_date, status);

-- Composite indexes for common queries
CREATE INDEX idx_company_industry_active 
ON companies(industry, is_active);

CREATE INDEX idx_contact_company_active 
ON contacts(company_id, is_active);
```

#### **Partitioning Strategy**
```sql
-- Partition companies by created_at year
CREATE TABLE companies_2023 PARTITION OF companies
...
This Mini CRM application demonstrates a modern, scalable approach to building enterprise applications. The technical decisions balance:

1. **Developer Experience**: Vue 3 Composition API, Django admin, comprehensive tooling
2. **User Experience**: Responsive design, real-time updates, keyboard shortcuts
3. **Performance**: Database optimization, caching strategies, code splitting
4. **Security**: JWT authentication, soft deletes, audit logging
5. **Maintainability**: Clean architecture, comprehensive testing, clear documentation

The application is production-ready with clear upgrade paths for additional features, scaling requirements, and integration opportunities.
...
- ✅ Django REST Framework with best practices
- ✅ Comprehensive testing suite
- ✅ Production-ready deployment configuration
- ✅ Scalable architecture with clear upgrade paths

This solution provides a solid foundation for a CRM system that can grow from a small team tool to an enterprise-grade platform.